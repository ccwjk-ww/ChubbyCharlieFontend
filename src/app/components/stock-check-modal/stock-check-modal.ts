...
import { MatButtonModule } from '@angular/material/button';
import { MatIconModule } from '@angular/material/icon';
import { StockCheckDetail, IngredientDetail, StockAllocationDetail } from '../../services/order.service';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MatDialogModule, MatButtonModule, MatIconModule],
  templateUrl: './stock-check-modal.html',
  styleUrls: ['./stock-check-modal.css']
})
export class StockCheckModalComponent {
  constructor(
    public dialogRef: MatDialogRef<StockCheckModalComponent>,
  ...
  /**
   * ⭐ Check if ingredient uses Multi-Lot allocation
   */
  isMultiLot(ingredient: IngredientDetail): boolean {
    return !!(ingredient.stockAllocations && ingredient.stockAllocations.length > 0);
  }

  /**
   * ⭐ Get total available quantity for Multi-Lot
   */
  getTotalAvailable(ingredient: IngredientDetail): number {
    if (!ingredient.stockAllocations) return 0;

    return ingredient.stockAllocations.reduce((total, allocation) => {
      return total + (allocation.availableQuantity || 0);
    }, 0);
  }

  /**
   * ⭐ Get total allocated quantity for Multi-Lot
   */
  getTotalAllocated(ingredient: IngredientDetail): number {
    if (!ingredient.stockAllocations) return 0;

    return ingredient.stockAllocations.reduce((total, allocation) => {
      return total + allocation.allocatedQuantity;
    }, 0);
  }
}
